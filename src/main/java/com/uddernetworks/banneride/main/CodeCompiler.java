...
public class CodeCompiler {
    ...
    public class MyDiagnosticListener implements DiagnosticListener<JavaFileObject> {
        ...
        public void report(Diagnostic<? extends JavaFileObject> diagnostic) {
            ...
            BannerGrid imageClass = imageClassHashMap.get(packageName);
    ...
    public Map<BannerGrid, List<Diagnostic<? extends JavaFileObject>>> compileAndExecute(Player player, BannerGrid bannerGrid, File jarFile, File otherFiles, File classOutputFolder, File libsFolder, boolean execute) throws IOException {
        ...
        String classPackage = (bannerGrid.getCode().trim().startsWith("package") ? bannerGrid.getCode().trim().substring(8, bannerGrid.getCode().trim().indexOf(";")) : "");
        String[] spaces = bannerGrid.getCode().trim().split(" ");
        ...
        imageClassHashMap.put(classPackage + "." + className, bannerGrid);

        filesList.add(getJavaFileObject(bannerGrid.getCode(), classPackage, className));
        ...
        player.sendMessage(ChatColor.GOLD + "Compiled in " + String.valueOf((System.currentTimeMillis() - start)) + "ms");
        ...
        player.sendMessage(ChatColor.GOLD + "Packaged jar in " + (System.currentTimeMillis() - start) + "ms");
        ...
        player.sendMessage(ChatColor.GOLD + "Executed in " + (System.currentTimeMillis() - start) + "ms");
